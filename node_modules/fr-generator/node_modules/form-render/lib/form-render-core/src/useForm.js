...
var useForm = function useForm(props) {
  ...
  (0, _react.useEffect)(function () {
    if (schemaRef.current) {
      ...
      if (firstMount) {
        _flatten.current = (0, _utils.flattenSchema)(schemaRef.current);
        setState({
          firstMount: false
        });
      } else {
  ...
  }, [JSON.stringify(schemaRef.current), JSON.stringify(_data.current), firstMount]); // All form methods are down here ----------------------------------------------------------------
  // 两个兼容 0.x 的函数
  ...
  var setSchema = function setSchema(settings) {
    var newFlatten = (0, _utils.clone)(_finalFlatten.current);
  ...
  var setValues = function setValues(newFormData) {
    var newData = (0, _processData.transformDataWithBind2)(newFormData, _finalFlatten.current);
