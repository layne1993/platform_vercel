...
import { getDescriptorSimple, dataToKeys, destructDataPath, getDataPath, isExpression, parseSingleExpression, isObject, allPromiseFinish, removeDups } from './utils';
...
var getAllPaths = function getAllPaths(paths, flatten) {
  ...
  uniqueResult.forEach(function (result) {
    ...
    if (flatten[id]) {
      var children = flatten[id].children;
      ...
      res = [].concat(_toConsumableArray(res), _toConsumableArray(childrenWithIndex));
    }
  });
...
export var validateAll = function validateAll(_ref2) {
  ...
  var allPaths = getAllPaths(paths, flatten); // console.log('allPaths', allPaths);
