...
// beta版本注：由于目前 @umijs/server 没有导出 responseInterceptor 函数，所以这里只是临时导入作为 beta 版本测试，实际上线后需要修改 umi 的导出，而非再次引入 http-proxy-middleware
...
function _default(api) {
  ...
  api.modifyDefaultConfig(memo => {
    ...
    const initialSlaveOptions = (0, _objectSpread2().default)((0, _objectSpread2().default)({
      devSourceMap: true
    ...
    const modifiedDefaultConfig = (0, _objectSpread2().default)((0, _objectSpread2().default)({}, memo), {}, {
      ...
      runtimeHistory: {},
      qiankun: (0, _objectSpread2().default)((0, _objectSpread2().default)({}, memo.qiankun), {}, {
  ...
  api.modifyPublicPathStr(publicPathStr => {
    ...
    const shouldNotModifyRuntimePublicPath = (api.config.qiankun || {}).slave.shouldNotModifyRuntimePublicPath;
  ...
  api.chainWebpack((config, {
    ...
    const shouldNotAddLibraryChunkName = (api.config.qiankun || {}).slave.shouldNotAddLibraryChunkName;
  ...
  api.addMiddlewares( /*#__PURE__*/(0, _asyncToGenerator2().default)(function* () {
    return /*#__PURE__*/function () {
      var _ref2 = (0, _asyncToGenerator2().default)(function* (req, res, next) {
        var _api$config$qiankun$s, _yield$api$applyPlugi;

        const masterEntry = api.config.qiankun && ((_api$config$qiankun$s = api.config.qiankun.slave) === null || _api$config$qiankun$s === void 0 ? void 0 : _api$config$qiankun$s.masterEntry);

        const _ref3 = (_yield$api$applyPlugi = yield api.applyPlugins({
          key: 'shouldProxyToMaster',
          type: api.ApplyPluginsType.modify,
          initialValue: {
            proxyToMasterEnabled: true,
            req
          }
        })) !== null && _yield$api$applyPlugi !== void 0 ? _yield$api$applyPlugi : {},
              proxyToMasterEnabled = _ref3.proxyToMasterEnabled;

        if (masterEntry && proxyToMasterEnabled) {
          const appName = api.pkg.name;
          (0, _assert().default)(appName, '[@umijs/plugin-qiankun]: You should have name in package.json');
          return (0, _server().createProxyMiddleware)(pathname => pathname !== '/local-dev-server', {
            target: masterEntry,
            secure: false,
            ignorePath: true,
            ...
            onProxyRes: (0, _httpProxyMiddleware().responseInterceptor)( /*#__PURE__*/function () {
              var _ref4 = (0, _asyncToGenerator2().default)(function* (responseBuffer, proxyRes, req, res) {
                ...
                let html = originalHtml.replace('<head>', `<head><script type="extra-qiankun-config">${JSON.stringify({
                  master: {
                    apps: [{
                      name: appName,
                      entry: microAppEntry,
            ...
            onError(err, _, res) {
              ...
              res.end(`[@umijs/plugin-qiankun] 代理到 ${masterEntry} 时出错了，请尝试 ${masterEntry} 是否是可以正常访问的，然后重新启动项目试试。(注意如果出现跨域问题，请修改本地 host ，通过一个和主应用相同的一级域名的域名来访问 127.0.0.1)`);
            }
...
function useLegacyMode(api) {
  ...
  const _ref5 = options || {},
        _ref5$keepOriginalRou = _ref5.keepOriginalRoutes,
        keepOriginalRoutes = _ref5$keepOriginalRou === void 0 ? false : _ref5$keepOriginalRou;
